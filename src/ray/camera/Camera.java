...
public class Camera {
	 ...
	 * these vectors to describe the camera orientation. <<<<<<< HEAD
	 * 
	 * =======
	 * 
	 ...
	 * @param inYFOV
	 *            the new horizontal field of view in radians
	 */
	public Camera(Point3 eye, Point3 target, Vector3 up, double inXFOV,
			double inYFOV) {
	...
	public void setAspectRatioAndYFOV(double inYFOV, double inAspectRatio) {
		...
		yFOV = inYFOV;
		xFOV = 2 * Math.toDegrees(Math.atan(Math.tan(Math.toRadians(yFOV) / 2)
				* inAspectRatio));
		tanHalfYFOV = Math.tan(Math.toRadians(yFOV) / 2);
	 ...
	 * @param inTarget
	 *            the new target of the camera
	 * @param inUp
	 *            the new up direction of the camera
	 */
	public void lookAt(Point3 inEye, Point3 inTarget, Vector3 inUp) {
		...
		back.sub(inEye, inTarget); // The back vector points from the target to
									// the eye
		...
		up.cross(back, right); // but we need to set up to the cross of foward
								// and right in case the forward and right
								// vectors
	 ...
	 *            The output ray
	 * @param inU
	 *            The u coord of the image point
	 * @param inV
	 *            The v coord of the image point
	 */
	public void getRay(Ray outRay, double inU, double inV) {
		...
		outRay.direction.scaleAdd(-inU * tanHalfXFOV, right); // Move the
																// direction
		// along the right/left axis
		outRay.direction.scaleAdd(-inV * tanHalfYFOV, up); // Move the direction
															// along
		// the up/down axis
	...
	/**
	 * Used internally by the parser to set the original value of the camera.
	 * Use lookAt instead.
	 * 
	 * @deprecated
	 * @param cacheEye
