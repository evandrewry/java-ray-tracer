 ...
 * 
 * @author arbree Aug 18, 2005 FloatRGBImage.java Copyright 2005 Program of
 *         Computer Graphics, Cornell University
 */
public class Image {

	/**
	 * Max number of units in a channel expressed as a float to cause int to
	 * float conversion
	 */
	...
	/**
	 * Set the size of the image by recreating it. Destorys all current image
	 * data.
	 * 
	 * @param newWidth
	 *            width
	...
	/**
	 * Return a BufferedImage (Java's internal image format) version of this
	 * image. The returned image is backed by the same data as this image, so
	 * changes to that image will affect this image. This is needed to make the
	 * updates to the display window fast and for image IO. It is not
	...
	public BufferedImage asBufferedImage() {
		...
		// sample model describes how to find pixels and pixel components in the
		// data buffer
		PixelInterleavedSampleModel sm = new PixelInterleavedSampleModel(
				db.getDataType(), width, height, pixelFloats, width
						* pixelFloats, bandOffsets);
		...
		ComponentColorModel cm = new ComponentColorModel(cs, false, true,
	...
	/**
	 * Creates an image using the data in the input bufferedImage. The resulting
	 * image has a copy of the data contained in inImage, as floats.
	 * 
	 * @param imageClass
	 *            the class type of the returned image
	 * @param inImage
	 *            the bufferedImage data
	 */
	...
	/**
	 * Write this image to the filename. The output is always written as a PNG
	...
	public void write(String fileName) {
		...
		// from the
		// image data, so we have to recreate the image to produce one for
		// output.
		...
		try {
			...
			System.out.println("Error occured while attempting to write file: "
					+ fileName);
