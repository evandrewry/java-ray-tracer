 ...
 * 
 * @author arbree Aug 19, 2005 MeshTriangle.java Copyright 2005 Program of
...
public class MeshTriangle extends Surface {
	...
	public void setMaterial(Material material) {
		throw new Error(
				"The material for a individual triangle in a Mesh cannot be set, set the material in the mesh object.");
	}

	public void updateArea() {
		Vector3 u = new Vector3(), v = new Vector3(), n = new Vector3();
		u.set(mesh.verts[3 * v1] - mesh.verts[3 * v0], mesh.verts[3 * v1 + 1]
				- mesh.verts[3 * v0 + 1], mesh.verts[3 * v1 + 2]
				- mesh.verts[3 * v0 + 2]);
		v.set(mesh.verts[3 * v2] - mesh.verts[3 * v0], mesh.verts[3 * v2 + 1]
				- mesh.verts[3 * v0 + 1], mesh.verts[3 * v2 + 2]
				- mesh.verts[3 * v0 + 2]);
		n.cross(u, v);
		area = 0.5 * n.length();
		oneOverArea = 1. / area;
	 ...
	 * reads the vertex data from the mesh array and computes the normals from
	 * the normal array (if present)
	 * 
	 * The implemented method closely follows the Cramer's rule method described
	 * on page 208 of Shirley.
	 * 
	...
	public boolean intersect(IntersectionRecord outRecord, Ray ray) {
		...
		// If the intersection is out of this range it doesn't fall on the
		// triangle
		...
		outRecord.frame.o.set(weight0 * v0x + beta * v1x + gamma * v2x, weight0
				* v0y + beta * v1y + gamma * v2y, weight0 * v0z + beta * v1z
				+ gamma * v2z);

		// If the normals array is empty use the old calculation
		if (mesh.normals == null) {

			// Calculate the normal of the triangle. Again this could be
			// precomputed,
		...
		// coordinates
			...
			// Compute normal
			outRecord.frame.w.set(weight0 * n0x + beta * n1x + gamma * n2x,
					weight0 * n0y + beta * n1y + gamma * n2y, weight0 * n0z
							+ beta * n1z + gamma * n2z);
			outRecord.frame.initFromW();
		...
		if (mesh.texcoords != null) {
			...
			// Compute normal
			outRecord.texCoords.set(weight0 * t0x + beta * t1x + gamma * t2x,
					weight0 * t0y + beta * t1y + gamma * t2y);
	...
	public void getCenter(Point3 outCenter) {
		...
		outCenter.set(mesh.verts[3 * v0] + mesh.verts[3 * v1]
				+ mesh.verts[3 * v1], mesh.verts[3 * v0 + 1]
				+ mesh.verts[3 * v1 + 1] + mesh.verts[3 * v1 + 1],
				mesh.verts[3 * v0 + 2] + mesh.verts[3 * v1 + 2]
						+ mesh.verts[3 * v1 + 2]);
		outCenter.scale(1 / 3.);
	...
	public void addToBoundingBox(AxisAlignedBoundingBox inBox) {
		inBox.add(mesh.verts[3 * v0], mesh.verts[3 * v0 + 1],
				mesh.verts[3 * v0 + 2]);
		inBox.add(mesh.verts[3 * v1], mesh.verts[3 * v1 + 1],
				mesh.verts[3 * v1 + 2]);
		inBox.add(mesh.verts[3 * v2], mesh.verts[3 * v2 + 1],
				mesh.verts[3 * v2 + 2]);
	}
	...
	public boolean chooseSamplePoint(Point3 p, Point2 seed,
		...
		lRec.frame.o.set(mesh.verts[3 * v0], mesh.verts[3 * v0 + 1],
				mesh.verts[3 * v0 + 2]);
	...
	public double pdfSamplePoint(IntersectionRecord iRec,
			LuminaireSamplingRecord lRec) {
		return oneOverArea;
	}
