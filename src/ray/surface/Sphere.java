...
public class Sphere extends Surface {
	 ...
	 * 
	 * @param newCenter
	 *            The center of the new sphere.
	 * @param newRadius
	 *            The radius of the new sphere.
	 * @param newMaterial
	 *            The material of the new sphere.
	 */
	...
	public boolean intersect(IntersectionRecord outRecord, Ray ray) {
		double t;
		Vector3 centerVect = new Vector3(ray.origin, this.center);
		double a = ray.direction.dot(ray.direction);
		double b = 2 * ray.direction.dot(centerVect);
		...
		outRecord.frame.o.set(inX, inY, inZ);
		outRecord.frame.w.set(x - this.center.x, y - this.center.y, z
				- this.center.z);
        outRecord.frame.initFromW();

		// W4160 TODO (A)
		// In this function, you need to test if the given ray intersect with a
		// sphere.
		// You should look at the intersect method in other classes such as
		// ray.surface.Triangle.java
		...
		//
		// Note: Although a ray is conceptually a infinite line, in practice, it
		// often has a length,
		// and certain rendering algorithm relies on the length. Therefore, here
		// a ray is a
		// segment rather than a infinite line. You need to test if the segment
		// is intersect
		// with the sphere. Look at ray.misc.Ray.java to see the information
		// provided by a ray.
