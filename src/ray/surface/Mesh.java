...
public class Mesh extends Surface {
	...
	private void setMeshData(float[] verts, int[] tris, float[] normals,
		...
		for (int i = 0; i < numTriangles; i++)
			triangles[i] = new MeshTriangle(this, tris[3 * i], tris[3 * i + 1],
					tris[3 * i + 2]);
	...
	public void setData(String fileName) {
		...
		try {
			...
			e.printStackTrace();
			throw new Error(
					"ray1.surface.Mesh.setData(): Error reading mesh file.");
		}
	}
	...
	public boolean intersect(IntersectionRecord outRecord, Ray ray) {
		...
		throw new Error(
				"Meshes cannot intersect rays.  Test for intersection against the individual mesh triangles.");
	...
	public void getCenter(Point3 outCenter) {
		throw new Error(
				"ray1.surface.Mesh.getCenter(): Should never be called.");
	}
	...
	public void addToBoundingBox(AxisAlignedBoundingBox inBox) {
		throw new Error(
				"ray1.surface.Mesh.addToBoundingBox(): Should never be called.");
	}
	 ...
	 * 
	 * @param outputMesh
	 *            the mesh to store the read data
	 * @param fileName
	...
	public boolean chooseSamplePoint(Point3 iRec, Point2 seed,
