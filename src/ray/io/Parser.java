 ...
 * for methods named, first, set<name> and then add<name>. If no such method is
 * found or if the found method does not take exactly one parameter, parsing
 ...
 * of the type node can be the fully qualified name for class (e.g.
 * ray.surface.Sphere), or just the name of a class in the same package as the
 * base type inferred from the parent method (e.g. type="Sphere" when the
 * addSurface method accepts type ray.surface.Surface, is shorthand for
 * type="ray.surface.Sphere"). Additionally, nodes can be named with the "name"
 ...
 * to index in the hash table nd the previously stored node with the matching
 ...
 * type="ray.material.Lambertian"> <diffuseColor>1 1 1</diffuseColor>
 * </material> <surface type="Sphere"> <material ref="white"/> <center>1 1
 * 1</center> <radius>1</radius> </surface> </scene>
 * 
 ...
 * The type of the <surface> tag is shorthand for type="ray.surface.Sphere".
 * 
 ...
 * 
 * @author arbree Aug 18, 2005 Parser.java Copyright 2005 Program of Computer
...
public class Parser {
	...
	/**
	 * Parser will assume all that non-absolute paths for mesh files are
	 * relative to the path of the xml file itself. This string stores that path
	...
	private Object parseObject(Class<?> c, Node n) {
		...
		if (typeAttribute != null) {
			...
			try {
				Class<?> possibleClass;
				// Try to look up the class; if it fails, try looking in the
				// package
