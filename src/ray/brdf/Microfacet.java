...
public class Microfacet implements BRDF {
	...
	/**
	 * Given the incident direction, reflection direction, compute the BRDF
	 * value. The w component fo Frame3 aligns with the surface normal
	 * 
	 * @param incDir
	 *            incident direction in canonical coordinates
	 * @param outDir
	 *            outgoing direction also in canonical coordinates
	 * @see ray.brdf.BRDF#evaluate(ray.math.Frame3, ray.math.Vector3,
	 *      ray.math.Vector3, ray.misc.Color)
	 */
	public void evaluate(Frame3 frame, Vector3 incDir, Vector3 outDir,
			Color outBRDFValue) {
		...
		double rs = fresnel.eval(dim) * shadowing.eval(incDir, outDir, frame.w)
				* normalDistri.eval(dmn) / (4 * din * don);
	 ...
	 * 
	 * @param seed
	 *            is uniformly distributed in square [0...1]
	 * @param fixedDir
	 *            outgoing direction is specified
	 * @param dir
	 *            incident direction is generated according to half angle
	 *            direction which is generated from random seed
	 */
	public void generate(Frame3 frame, Vector3 fixedDir, Vector3 dir,
		...
		outWeight.add(specularBrdfWeight * fresnel.eval(dim)
				* shadowing.eval(dir, fixedDir, frame.w) * nd / (4. * din));
		outWeight
				.scale(1. / (specularSamplingWeight * nd * dmn / (4. * dim) + (1. - specularSamplingWeight)
						* don / Math.PI));
	}

	/**
	 * Evaluate the PDF used by generate(), with respect to the solid angle
	 * measure. Given the <fixedDir>, what is the PDF value to select the
	 * direction at <dir>?
	 * 
	 * For uniform sample, the pdf is p(dir|uniform) = cos(theta)/PI For
	 * importance sample, the pdf is p(dir|importance) = Po So the overall
	 * p(dir) = p(dir, uniform)+p(dir, importance) = cos(theta)/PI * (1-weight)
	 * + Po * weight The value of Po = Pm *
	 * ||partial(half_angle)/partial(outDir)||
	 * 
	 * @param fixedDir
	 *            The fixed argument of the BRDF
	 * @param dir
	...
	public double pdf(Frame3 frame, Vector3 fixedDir, Vector3 dir) {
		...
		return specularSamplingWeight
				* (normalDistri.eval(dmn) * dmn / (4 * dim))
				+ (1. - specularSamplingWeight) * dir.dot(frame.w) / Math.PI;
	}

	private void seed2Dir(Point2 seed, Vector3 dir) {
		double theta = Math
				.atan(Math.sqrt(-roughnessSqr * Math.log(1 - seed.x)));
	...
	private class SmithShadowing {
		...
		public double eval(Vector3 vecIn, Vector3 vecOut, Vector3 vecN) {
			return monoDirShadowing(vecIn.dot(vecN))
					* monoDirShadowing(vecOut.dot(vecN));
		}

		private double monoDirShadowing(double co) {
			if (co < MathExt.DOUBLE_EPS)
				return 0;
			double ta = Math.sqrt(1. - co * co) / co;
			if (ta < MathExt.DOUBLE_EPS)
				return 1;
			double a = 1. / (roughness * ta);

			return 2. / (1. + MathExt.erf(a) + Math.exp(-a * a)
					* MathExt.M_1_SQRTPI);
		}
	}
	...
	private class BeckmannDistribution {
		...
		/**
		 * @param cosine
		 *            the cosine value of angle between normal direction and
		 *            half angle direction
		 * @return
		 */
		public double eval(double cosine) {
			if (cosine < MathExt.DOUBLE_EPS)
				return 0.;
			double c2 = cosine * cosine; // cosine square
			double t2 = (1. - c2) / c2; // tangent square
			return Math.exp(-t2 / roughnessSqr)
					/ (Math.PI * roughnessSqr * c2 * c2);
		}
	}
	...
	private class Fresnel {
		...
		/**
		 * @param cosine
		 *            The consine value of the angle between
		 * @return
		 */
		public double eval(double cosine) {
	...
	public static void main(String argv[]) {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < m; j++) {
				...
				// Generate a random direction and contribute to a pixel in the
				// test image
